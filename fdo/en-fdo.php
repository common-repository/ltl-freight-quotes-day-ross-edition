...
namespace EnDayrossFdo;

if (!class_exists('EnDayrossFdo')) {
    ...
    class EnDayrossFdo
    {
        ...
        static public function en_cart_package($en_package, $key)
        {
            ...
            self::$en_fdo_meta_data['plugin_name'] = 'dayross';
            $accessorials['residential'] = get_option('en_quote_settings_residential_delivery_dayross') == 'yes' ? true : false;
            $accessorials['liftgate'] = get_option('en_quote_settings_liftgate_delivery_dayross') == 'yes' ? true : false;
        ...
        static public function en_package_items($items)
        {
            ...
            foreach ($items as $item_key => $item_data) {
                $doNesting = $nestingPercentage = $nestingDimension = $nestedLimit = $nestedStackProperty = $isHazmatLineItem = $nmfc_number = $productId = $productName = $piecesOfLineItem = $lineItemPrice = $lineItemWeight = $lineItemLength = $lineItemWidth = $lineItemHeight = $ptype = $productType = $productSku = $productClass = $attributes = $variantId = '';
                $attributes = [];
                extract($item_data);
                ...
                $item = [
                    ...
                    'hazmat' => $isHazmatLineItem,
                    'isHazmatLineItem' => $isHazmatLineItem,
                    'doNesting' => $doNesting,
                    'nestingPercentage' => $nestingPercentage,
                    'nestingDimension' => $nestingDimension,
                    'nestedLimit' => $nestedLimit,
                    'nestedStackProperty' => $nestedStackProperty,
                ];
