jQuery(document).ready(function () {
    ...
    jQuery('#en_dayross_location_zip').on('change', function (e) {
        let en_post_data = {
            'action': 'en_dayross_get_location',
            'en_dayross_location_zip': jQuery('#en_dayross_location_zip').val()
        };

        let en_params = {
            en_ajax_loading_id: '#en_dayross_location_zip,#en_location_city,#en_location_state,#en_location_country',
        };

        en_ajax_request(en_params, en_post_data, en_dayross_get_location);
    });
    ...
    jQuery('.en_dayross_location_btn').on('click', function (e) {
        ...
        var tab = get_parameter_by_dayross('tab');

        let en_post_data = {
            'tab': tab,
            'action': 'en_dayross_location_save_form_data',
            'en_post_data': jQuery(".en_popup_location_form input").serialize()
        };

        let en_params = {
            en_ajax_loading_msg_btn: '.en_dayross_location_btn',
        };

        en_ajax_request(en_params, en_post_data, en_dayross_location_save_form_data);
    ...
    jQuery("#en_wd_origin_markup, ._en_product_markup").bind("cut copy paste",function(e) {
        e.preventDefault();
    });
    
    jQuery("#en_wd_origin_markup, ._en_product_markup").keypress(function (e) {
        if (!String.fromCharCode(e.keyCode).match(/^[-0-9\d\.%\s]+$/i)) return false;
    });

    jQuery("#en_wd_origin_markup, ._en_product_markup").keydown(function (e) {
    ...
    jQuery("#en_wd_origin_markup, ._en_product_markup").keyup(function (e) {
 ...
 */
if (typeof en_dayross_location_save_form_data != 'function') {
    function en_dayross_location_save_form_data(params, response) {
 ...
 */
if (typeof en_dayross_get_location != 'function') {
    function en_dayross_get_location(params, response) {
        let data = JSON.parse(response);
 ...
 */
if (typeof en_dayross_location_edit != 'function') {
    function en_dayross_location_edit(e, data, en_location_type) {
        e.preventDefault();
 ...
 * Get url detail
 */
if (typeof get_parameter_by_dayross != 'function') {
    function get_parameter_by_dayross(name) {
 ...
 */
if (typeof en_dayross_location_delete != 'function') {
    function en_dayross_location_delete(e, data, en_location_type, en_location_id) {
        e.preventDefault();
        en_popup_confirmation_location_delete_show();
        jQuery('.en_close_popup_location').on('click', function () {
            en_location_id = false;
        });
...
if (typeof en_location_confirm_delete != 'function') {
    function en_location_confirm_delete(data, en_location_type, en_location_id) {
        ...
        let en_post_data = {
            'tab': tab,
            'action': 'en_dayross_location_delete_row',
