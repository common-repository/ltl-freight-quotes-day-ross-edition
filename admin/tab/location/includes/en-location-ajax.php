...
namespace EnDayrossLocationAjax;

use EnDayrossDistance\EnDayrossDistance;
use EnDayrossDropshipTemplate\EnDayrossDropshipTemplate;
use EnDayrossWarehouse\EnDayrossWarehouse;
use EnDayrossWarehouseTemplate\EnDayrossWarehouseTemplate;

if (!class_exists('EnDayrossLocationAjax')) {

    class EnDayrossLocationAjax
    {

        public function __construct()
        {
            add_action('wp_ajax_nopriv_en_dayross_location_save_form_data', [$this, 'en_dayross_location_save_form_data']);
            add_action('wp_ajax_en_dayross_location_save_form_data', [$this, 'en_dayross_location_save_form_data']);

            add_action('wp_ajax_nopriv_en_dayross_get_location', [$this, 'en_dayross_get_location']);
            add_action('wp_ajax_en_dayross_get_location', [$this, 'en_dayross_get_location']);

            add_action('wp_ajax_nopriv_en_dayross_location_delete_row', [$this, 'en_dayross_location_delete_row']);
            add_action('wp_ajax_en_dayross_location_delete_row', [$this, 'en_dayross_location_delete_row']);
        }

        /**
         * Get response from api when we sent zip code
         */
        public function en_dayross_get_location()
        {
            if (isset($_POST['en_dayross_location_zip']) && strlen($_POST['en_dayross_location_zip']) > 4) {
                $get_address = EnDayrossDistance::get_address(sanitize_text_field($_POST['en_dayross_location_zip']), 'address');
                ...
                if (isset($decoded_address['results']) && is_array($decoded_address['results']) && count($decoded_address['results']) > 0) {
            ...
            echo wp_json_encode(['message' => EN_DAYROSS_709]);
            exit;
        }
         ...
         */
        public function en_dayross_location_delete_row()
        {
            global $wpdb;
            ...
            if ($location === 'warehouse') {
                $en_location_html = EnDayrossWarehouseTemplate::en_load();
                $en_target_location = '.en_location_warehouse_main_div';
            } else if ($location === 'dropship') {
                $en_location_html = EnDayrossDropshipTemplate::en_load();
         ...
         */
        public function en_dayross_location_save_form_data()
        {
            global $wpdb;
            $post_data = [];
            ...
            if (isset($post_data['id'], $post_data['location'])) {
                ...
                if ($location === 'warehouse') {
                    ...
                    $en_location_template_obj = new EnDayrossWarehouseTemplate();
                    $en_target_location = '.en_location_warehouse_main_div';
                    $validate = ['zip', 'city', 'state', 'country', 'location'];
                } else {
                    $location_step = 'Dropship';
                    $en_location_template_obj = new EnDayrossDropshipTemplate();
                    $en_target_location = '.en_location_dropship_main_div';
                ...
                $en_location_data = EnDayrossWarehouse::get_data($en_intersected_data);
