...
//      We can use hook for add new location field from other plugin add-on
            return apply_filters('en_dayross_add_location', $en_location_data);
        }
        ...
        static public function en_arrange_location_table_row($en_location_list, $en_data, $location_bol, $disabled_plan = '')
        {
            ...
            foreach ($en_location_list as $key => $location) {
                ...
                echo '<a href="javascript(0)" onclick="return en_dayross_location_edit(event, this,' . esc_attr($location_bol) . ')"> <img src = "' . EN_DAYROSS_DIR_FILE . '/admin/tab/location/assets/images/edit.png" title = "Edit" ></a>';
                echo '<a href="javascript(0)" onclick="return en_dayross_location_delete(event, this,' . esc_attr($location_bol) . ' , ' . esc_attr($en_location_id) . ')"> <img  src = "' . EN_DAYROSS_DIR_FILE . '/admin/tab/location/assets/images/delete.png" title = "Delete" ></a>';
                echo '</td>';
        ...
        static public function en_load()
        {
            ...
            echo '<input type="submit" value="Save" class="en_dayross_location_btn button-primary">';
            echo '</form>';
            echo '</div>';
            echo '</div>';

            echo \EnDayrossWarehouseTemplate\EnDayrossWarehouseTemplate::en_load();
            echo \EnDayrossDropshipTemplate\EnDayrossDropshipTemplate::en_load();
        }
        ...
        {
            $instore_pickup_tab = !empty($_REQUEST['tab']) ? sanitize_text_field($_REQUEST['tab']) : '';
            if (isset($instore_pickup_tab)) {
                $instore_pickup = apply_filters($instore_pickup_tab . "_plans_suscription_and_features", 'instore_pickup_local_delivery');
                if (is_array($instore_pickup) && count($instore_pickup) > 0) {
                    self::$plan_required = apply_filters($instore_pickup_tab . "_plans_notification_link", $instore_pickup);
