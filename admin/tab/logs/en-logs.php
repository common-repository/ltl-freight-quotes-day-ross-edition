...
if (!class_exists('EnDayrossLfqLogs')) {
    class EnDayrossLfqLogs
    {
        ...
        {
            $obj_classs = new \EnDayrossCurl\EnDayrossCurl();
            $data = array(
                'serverName' => EN_DAYROSS_SERVER_NAME,
                'licenseKey' => get_option('en_connection_settings_license_key_dayross'),
                'lastLogs' => '25',
                'carrierName' => 'dayross',
            );
            ...
            $url = EN_DAYROSS_ROOT_URL_QUOTES . '/request-log/index.php';
            $logs = $obj_classs->en_dayross_sent_http_request($url, $data, 'POST');
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    if (isset($request['carrierMode']) && $request['carrierMode'] == 'test') {
                        continue;
                    }
                    ...
                    foreach ($formatted_info as $key => $value) {
                        $formattedSenderAddress .= '<p>' . $value['origin']['senderCity'] . ', ' . $value['origin']['senderState'] . ' ' . $value['origin']['senderZip'] . '</p>';
                    ...
                    foreach ($hidden_formatted_info as $key => $value) {
                        $hiddenSenderAddress .= '<p>' . $value['origin']['senderCity'] . ', ' . $value['origin']['senderState'] . ' ' . $value['origin']['senderZip'] . '</p>';
                    ...
                    $en_address = $address;
                    $class_name = 'dayross-log-' . $key . rand(1, 100);
