jQuery(document).ready(function () {
    ...
    // Overlength Fees
    en_default_to_text_fields('#en_quote_settings_overlength_range_1_dayross', 85);
    en_default_to_text_fields('#en_quote_settings_overlength_range_2_dayross', 150);
    en_default_to_text_fields('#en_quote_settings_overlength_range_3_dayross', 250);
    en_default_to_text_fields('#en_quote_settings_overlength_range_4_dayross', 285);
    en_default_to_text_fields('#en_quote_settings_overlength_range_5_dayross', 770);

    // Common settings.
    jQuery('#en_connection_settings_account_number_dayross').closest('table').attr('id', 'en_dayross_connection_settings');
    jQuery('#en_quote_settings_custom_label_dayross').closest('table').attr('id', 'en_dayross_quote_settings');
    jQuery("#order_shipping_line_items .shipping .display_meta").css('display', 'none');

    // Carriers Tab
    if (jQuery('.en_dayross_carriers').length > 0) {
        // Making the function to check when no checkbox was checked.
        jQuery('.en_dayross_carriers').next('.submit').find('.button-primary, .is-primary').on('click', function (event) {
            jQuery('.en_settings_message').remove();
            let en_dayross_carrier = jQuery('.en_dayross_carriers .en_dayross_carrier').is(':checked');
            if (!en_dayross_carrier) {
    ...
    // Quote Settings Tab
    if (jQuery('#en_dayross_quote_settings').length > 0) {
        ...
        if (!jQuery('#auto_residential_delivery_plan_auto_renew').length) {
            jQuery(".en_woo_addons_liftgate_with_auto_residential_dayross").attr("disabled", true);
        }

        // Liftgate accessorials action perfrom on click
        jQuery(".liftgate_accessorial_action").on("click", function () {
            var id = jQuery(this).attr("id");
            if (id == "en_quote_settings_liftgate_delivery_dayross") {
                jQuery("#dayross_liftgate_delivery_as_option").prop({checked: false});
                jQuery("#en_woo_addons_liftgate_with_auto_residential").prop({checked: false});

            } else if (id == "dayross_liftgate_delivery_as_option" ||
                id == "en_woo_addons_liftgate_with_auto_residential") {
                jQuery("#en_quote_settings_liftgate_delivery_dayross").prop({checked: false});
            }
        });

        jQuery("#en_quote_settings_residential_delivery_dayross," +
            ...
            "#en_quote_settings_availability_auto_residential_dayross," +
            "#en_quote_settings_liftgate_delivery_dayross," +
            "#dayross_liftgate_delivery_as_option," +
            "#en_quote_settings_availability_liftgate_dayross," +
        ...
        jQuery('#en_dayross_quote_settings').parents().find('.button-primary, .is-primary').on('click', function (event) {

            let en_validate_settings = {};
            let en_data_error = true;

            en_validate_settings['#en_quote_settings_overlength_range_1_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
                'en_max_length': 7,
                'en_error_msg': 'Overlength fees format should be 100.20 and only 2 digits are allowed after decimal point.',
            };

            en_validate_settings['#en_quote_settings_overlength_range_2_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
            ...
            en_validate_settings['#en_quote_settings_overlength_range_3_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
            ...
            en_validate_settings['#en_quote_settings_overlength_range_4_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
            ...
            en_validate_settings['#en_quote_settings_overlength_range_5_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
            ...
            en_validate_settings['#en_quote_settings_handling_fee_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
            ...
            en_validate_settings['#en_quote_settings_handling_unit_weight_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
            ...
            en_validate_settings['#en_quote_settings_max_handling_unit_weight_dayross'] = {
                'en_data_type': 'isNumeric',
                'en_after_decimal': 2,
                'en_add_percentage': false,
                'en_minus_sign': false,
    ...
    if (jQuery('#en_dayross_connection_settings').length > 0) {

        jQuery('#en_connection_settings_username_dayross').attr('title', 'Email');
        jQuery('#en_connection_settings_password_dayross').attr('title', 'Passsword');
        jQuery('#en_connection_settings_account_number_dayross').attr('title', 'Bill To Account Number');
        jQuery('#en_connection_settings_license_key_dayross').attr('title', 'Eniture API Key');

        jQuery('#en_dayross_connection_settings').before("<div class='en_warning_message'><p>Note! You must have an LTL Freight enabled Day & Ross account to use this application. If you do not have one, call 877.226.9950, or <a href='https://www.dayross.com/' target='_blank' >register</a> online.</p></div>");
         ...
         */
        jQuery('#en_dayross_connection_settings input[type="text"]').each(function () {
        ...
        jQuery('.en_dayross_connection_settings .woocommerce-save-button').text('Save Changes');
        jQuery('.woocommerce-save-button').before('<button name="en_dayross_test_connection" class="button-primary en_dayross_test_connection is-primary components-button" id="en_dayross_test_connection" type="submit" value="Test Connection">Test Connection</button>');

        jQuery('.en_dayross_connection_settings .button-primary, .en_dayross_connection_settings .is-primary').on('click', function (event) {
            let validate = en_validate_input('#en_dayross_connection_settings');

            if (validate === false) {
                return false;
            }

            if (event.target.id === 'en_dayross_test_connection') {
                let postForm = {
                    'action': 'en_dayross_test_connection',
                    'en_post_data': window.btoa(jQuery('#en_dayross_connection_settings input').serialize()),
                };

                let params = {
                    en_ajax_loading_id: '#en_connection_settings_username_dayross,#en_connection_settings_password_dayross,#en_connection_settings_account_number_dayross,#en_connection_settings_license_key_dayross',
                };
    ...
    // fdo va
    jQuery('#fd_online_id_dayross').click(function (e) {
        var postForm = {
            'action': 'dayross_fd',
            'company_id': jQuery('#freightdesk_online_id').val(),
            'disconnect': jQuery('#fd_online_id_dayross').attr("data")
        }
        var id_lenght = jQuery('#freightdesk_online_id').val();
        var disc_data = jQuery('#fd_online_id_dayross').attr("data");
        ...
        jQuery.ajax({
            ...
            beforeSend: function () {
                jQuery('#freightdesk_online_id').css('background',
                    'rgba(255, 255, 255, 1) url("' + EN_DAYROSS_DIR_FILE + '' +
            ...
            success: function (data_response) {
                ...
                if((typeof (fd_data.is_valid) != 'undefined' && fd_data.is_valid == false) || (typeof (fd_data.status) != 'undefined' && fd_data.is_valid == 'ERROR')) {
                    ...
                    jQuery('#con_dis').append('<a href="#" id="fd_online_id_dayross" data="disconnect" class="button-primary">Disconnect</a>')
                }
            }
        });
        e.preventDefault();
     ...
     */
    jQuery("#en_dayross_total_carriers").on('click', function () {
        jQuery('.en_dayross_carriers input:checkbox').not(this).prop('checked', this.checked);
    });

    /**
     * carrier checkbox
     */
    jQuery(".en_dayross_carrier").on('click', function () {
        en_dayross_carriers();
    });
...
if (typeof en_default_to_text_fields != 'function') {
    function en_default_to_text_fields(id, price) {
        if (jQuery(id).length && !jQuery(id).val().length > 0) {
            jQuery(id).val(price);
        }
    }
}
...
if (typeof en_action_test_connection != 'function') {
    function en_action_test_connection(params, response) {
        ...
        switch (data_severity) {
            ...
            case 'ERROR':
                en_message = data.Message;
                if (typeof en_message === "object" && !Array.isArray(en_message) && en_message !== null) {
                    if (typeof en_message['0'] !== 'undefined') {
                        en_message = en_message['0'];
                    }
                }
                break;
            default:
 ...
 */
if (typeof en_dayross_carriers != 'function') {
    function en_dayross_carriers() {
        if (jQuery('.en_dayross_carrier:checked').length == jQuery('.en_dayross_carrier').length) {
            jQuery('#en_dayross_total_carriers').prop('checked', true);
        } else {
            jQuery('#en_dayross_total_carriers').prop('checked', false);
        }
    }
}
