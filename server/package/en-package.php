...
namespace EnDayrossPackage;

use EnDayrossDistance\EnDayrossDistance;
use EnDayrossProductDetail\EnDayrossProductDetail;
use EnDayrossReceiverAddress\EnDayrossReceiverAddress;
use EnDayrossWarehouse\EnDayrossWarehouse;

/**
 * Get items detail from added product in cart|checkout page.
 * Class EnDayrossPackage
 * @package EnDayrossPackage
 */
if (!class_exists('EnDayrossPackage')) {
    ...
    class EnDayrossPackage
    {
        ...
        {
            $product_detail_obj = new EnDayrossProductDetail();
            ...
            self::$receiver_address = EnDayrossReceiverAddress::get_address();
                ...
                if (isset($product['data'])) {
                    ...
                    switch ($dimension_unit) {
                        case (isset($calculate_dimension[$dimension_unit])):
                            $get_height = round($product_data->get_height() * $calculate_dimension[$dimension_unit], 2);
                            $get_length = round($product_data->get_length() * $calculate_dimension[$dimension_unit], 2);
                            $get_width = round($product_data->get_width() * $calculate_dimension[$dimension_unit], 2);
                            break;
                        default;
                    ...
                    $dr_plan = get_option('en_dayross_plan_number');
                        
                    if ($dr_plan != 0 && $dr_plan != 1) {
                        $haz_array = get_post_meta(self::$post_id, '_hazardousmaterials');
                        isset($haz_array[0]) && $haz_array[0] == 'yes' ? $haz_status = 'Y' : $haz_status = '';
                    }
                    $doNesting = '';
                    $nested_percentage = 0;
                    $nested_dimension = 0;
                    $max_nested_items = 0;
                    $nested_staking_property = 0;
                    if ($dr_plan == 3) {
                        ...
                        !empty($nested_dim) ? $nested_dimension = $nested_dim[0] : $nested_dimension = 0;
                        ...
                        !empty($nested_staking) ? $nested_staking_property = $nested_staking[0] : $nested_staking_property = 0;
                    }
                    $product_title = str_replace(array("'", '"'), '', $product_data->get_title());
                    $attr_array = [];
                    $p_attr = $product_data->get_attributes();

                    if (!empty($p_attr) && is_array($p_attr)) {

                        foreach ($p_attr as $key => $value) {
                            if (is_object($value) && isset($value['options']) && is_array($value['options'])) {
                                $attr_array[$key] = str_replace(array("'", '"'), "", $value['options']);
                            } else {
                                if (!is_object($value)) {
                                    $attr_array[$key] = str_replace(array("'", '"'), "", $value);
                                }
                            }
                        }
                    }
                    ...
                    $get_freight = self::en_dayross_get_freight_class($product, $product_data);
                    ...
                    $product_item = [
                        ...
                        'hazardousMaterial' => $haz_status,
                        'hazardous_material' => $haz_status,
                        'hazmat' => $haz_status,
                        ...
                        'lineItemClass' => $freightClass_ltl_gross,
                    ];
                    ...
                    $origin_zip_code = '';
                    $shipment_type = EN_DAYROSS_DECLARED_FALSE;
                    // Micro Warehouse
                    ...
                    foreach ($en_product_fields as $en_field_key => $en_custom_product) {
                        ...
                        $is_line_item = self::$post_id;
                        $is_plans = self::en_sanitize_package('plans', '');

                        if (is_callable(array(self::class, $en_function_trigger)) && strlen($is_line_item) > EN_DAYROSS_DECLARED_ZERO) {
                            ...
                            $suscription_and_features = apply_filters(
                                "dayross_plans_suscription_and_features", $is_plans
                            );
                            ...
                            (!is_array($en_location_value) &&
                                strlen($en_location_value) > EN_DAYROSS_DECLARED_ZERO) ? $product_item[$is_line_item] = $en_location_value : "";
                            ...
                            if (isset($en_location_value['senderZip']) &&
                                ...
                                switch (EN_DAYROSS_DECLARED_TRUE) {
                                    case $shipping_class == 'ltl_freight':
                                        $shipment_type = EN_DAYROSS_DECLARED_TRUE;
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        self::$en_request['LTL_FREIGHT'] = EN_DAYROSS_DECLARED_ONE;
                                        break;
                                    case $shipment_weight > EN_DAYROSS_SHIPMENT_WEIGHT_EXCEEDS_PRICE && EN_DAYROSS_SHIPMENT_WEIGHT_EXCEEDS == 'yes':
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        $shipment_type = EN_DAYROSS_DECLARED_TRUE;
                                        $product_weight < EN_DAYROSS_SHIPMENT_WEIGHT_EXCEEDS_PRICE ?
                                            self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_DAYROSS_DECLARED_TRUE : "";
                                        break;
                                    case $product_weight < EN_DAYROSS_SHIPMENT_WEIGHT_EXCEEDS_PRICE:
                                        $shipment_type = EN_DAYROSS_DECLARED_TRUE;
                    ...
                    if ($shipment_type && strlen($origin_zip_code) > 0) {
                        self::$en_request['product_name'][$origin_zip_code][] = $product_quantity . " x " . $product_title;
                        self::$en_request['shipment_weight'][$origin_zip_code] = $shipment_weight;
                        self::$en_request['commdityDetails'][$origin_zip_code][$is_line_item] = $product_item;
        ...
        {
            $reasons = !self::$shipment_type ? array_merge($reasons, [EN_DAYROSS_712]) : $reasons;
            return (!self::$origin_zip_code > 0) ? array_merge($reasons, [EN_DAYROSS_713]) : $reasons;
        }
        ...
        static public function en_filter_shipment()
        {
            ...
            self::$en_request = apply_filters("en_woo_addons_carrier_service_quotes_request", self::$en_request, EN_DAYROSS_SHIPPING_NAME);
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['LTL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['LTL'] = EN_DAYROSS_DECLARED_TRUE;
        }
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['SMALL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_DAYROSS_DECLARED_TRUE;
        }
        ...
        static public function en_is_instore_pickup_enabled($location, $sender_location_address)
        {
            ...
            $suscription_and_features = apply_filters(
                "dayross_plans_suscription_and_features", 'instore_pickup_local_delivery'
            );
         ...
         */
        static public function en_dayross_get_location($location_id = '')
        {
            ...
            $en_location = EnDayrossWarehouse::get_data($en_where_clause);
            // Micro Warehouse
            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
            ...
            if (!empty($en_location) && is_array($en_location)) {
                if (count($en_location) == 1) {
                    ...
                    $get_address = json_decode(
                        EnDayrossDistance::get_address($en_location, $en_access_level, $receiver_address), true);
                    ...
                    switch ($enable_dropship) {
                        case 'yes':
                            ...
                            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                                ...
                                if(!empty($locations_dropship)) {
                                    self::$get_minimum_warehouse = self::en_dayross_get_location();
                                    return self::$get_minimum_warehouse;
                                }
                            }

                            $en_dayross_get_location = self::en_dayross_get_location(get_post_meta(self::$post_id, self::$product_key_name, true));

                            if (empty($en_dayross_get_location)) {
                                self::$get_minimum_warehouse = self::en_dayross_get_location();
                                return self::$get_minimum_warehouse;
                            }
                            return $en_dayross_get_location;
                        default:
                            self::$get_minimum_warehouse = self::en_dayross_get_location();
                            return self::$get_minimum_warehouse;
                    }
        ...
        */
         ...
         * @return string
         */
        static public function en_dayross_get_freight_class($values, $_product)
        {
            if ($_product->get_type() == 'variation') {
                $variation_class = get_post_meta($values['variation_id'], '_ltl_freight', true);
                ...
                if ($variation_class == 0) {
                    ...
                    if ($variation_class > 0) {
                        $freightClass_ltl_gross = get_post_meta($values['variation_id'], '_ltl_freight', true);
                    } else {
